...
function ReservationsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Доступные столы:', tablesResponse);
        setTables(tablesResponse || []);

        const reservationsResponse = await api.getReservations(userId);
        console.log('Ответ API по бронированиям:', reservationsResponse);
    ...
    fetchData();
  }, [restaurantId, newReservation.reservation_start]);
  ...
  const handleNewTableChange = (e) => {
    setNewTable({ table_number: e.target.value });
  };
  ...
  const handleCreateTable = async () => {
    ...
    try {
      ...
      setTables((prev) => [...prev, { table_number: response.table.table_number }]);
  ...
  const handleCreateReservation = async () => {
    ...
    try {
      const reservationData = {
        ...
        reservation_start: newReservation.reservation_start,
      };
      console.log('Отправляемые данные для бронирования:', reservationData);
      const response = await api.createReservation(restaurantId, userId, reservationData);
  ...
  const handleDeleteReservation = async (reservationId) => {
    ...
    try {
      await api.deleteReservation(restaurantId, userId, reservationId);
      ...
      <div style={{ marginBottom: '20px', padding: '10px', border: '1px solid #ccc', borderRadius: '5px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <button
            ...
            disabled={isCreatingTable || !newTable.table_number}
            style={{
              padding: '5px 10px',
          ...
          {reservations.map((reservation) => (
            ...
            >
              <div>
                ...
                <p>Время: {new Date(reservation.reservation_start).toLocaleString()}</p>
